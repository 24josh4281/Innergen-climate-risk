...
| 일별 변수 | 3개 (tasmax, tasmin, pr) — 4SSP 전체 |
...
### 최종 마스터 출력 (scenarios_v2/output/)

| 파일 | 내용 | 형태 |
|------|------|------|
| `OCI_MASTER_ALL.csv` | **최종 통합 마스터** — 55개 기후 지표 전체 | 416×59 |
| `ph8_risk_score.csv` | 종합 리스크 스코어 (0-100) × 8차원 × 4SSP | 416×14 |
| `ph3_etccdi_4ssp.csv` | ETCCDI 13지수 × 4SSP × 8기간 × 13사업장 | 5408×7 |
| `ph7_exceedance_years.csv` | 온난화/극값 임계값 최초 도달 연도 | 52×19 |
| `ph7_warming_timeline.csv` | 5년 간격 온난화량 타임라인 | 780×5 |
| `ph6_seasonal_temp.csv` | 4계절 기온 × 4SSP × 8기간 | 52×99 |
| `ph6_heat_stress.csv` | JJA WBGT + Humidex + DI | 52×27 |
| `ph9_runoff.csv` | 지표유출 연간/계절별 | 52×35 |
| `ph9_soilmoisture.csv` | 토양수분 + 여름 결핍 지수 | 52×35 |
| `ph9_solar.csv` | 태양복사 (W/m² + kWh/m²/day) | 52×35 |
| `ph4_return_period.csv` | GEV 10/50/100년 재현기간 강수량 | 26×11 |
| `ph4_compound_events.csv` | 복합 폭염+건조 이벤트 확률 | 26×7 |
| `ph1_cdd_hdd.csv` | 냉난방도일(CDD/HDD) × 4SSP × 8기간 | 26×19 |
| `ph1_humidex.csv` | Humidex + Apparent Temp × 4SSP × 8기간 | 26×20 |
| `ph1_fwi.csv` | 화재기상지수(FWI) × 4SSP × 8기간 | 26×19 |

### FINAL 통합 출력 (output/)

| 파일 | 내용 | 형태 |
|------|------|------|
| `OCI_FINAL_4SSP_Decadal.csv` | 12동인 × 4SSP × 8기간 × 13사업장 | 156×37 |
| `OCI_FINAL_SpecificYears.csv` | 7변수 × 4SSP × 4연도(2030/35/40/45) | 364×7 |
| `OCI_FINAL_ETCCDI.csv` | ETCCDI 13지수 × 2SSP × 8기간 | 208×17 |
| `OCI_FINAL_Indices.csv` | Heat Index + 온난화속도 + SPEI | 26×23 |
...
| 우선순위 | 항목 | 상태 |
|----------|------|------|
| ✅ 완료 | Phase 1: CDD/HDD + Humidex + SPI-3 + FWI + P-E | **완료** |
| ✅ 완료 | Phase 2: SSP1-2.6 / SSP3-7.0 일별 CMIP6 추가 | **완료** |
| ✅ 완료 | Phase 3: 4-SSP ETCCDI 완전 세트 (5408행) | **완료** |
| ✅ 완료 | Phase 4: GEV 재현기간 + 복합극한 이벤트 | **완료** |
| ✅ 완료 | Phase 5: 통합 마스터 CSV 3종 생성 | **완료** |
| 🔄 진행 | 글로벌 11개 지역 월별 528파일 다운로드 | **진행중** |
| 🔄 예정 | 글로벌 11개 지역 일별 66파일 다운로드 | **예정** |
| 높음 | 사업장별 PDF 리포트 자동화 | 미착수 |
| 중간 | LitPop × 피해함수 → 예상손실(EAL) 산출 | 미착수 |
| 낮음 | ERA5 과거 데이터 완성 → baseline 설정 | 보류 |
| 낮음 | ERA5 CAPE 기반 우박 프록시 | 미착수 |

---

*Last updated: 2026-03-17 (Phase 1~5 완전 완료, 글로벌 확장 진행중)*
